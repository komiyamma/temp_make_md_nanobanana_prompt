---
description: Generate an image generation plan for a range of HTML files based on their content analysis.
---
...
2.  **Iterate and Analyze**:
    - For each `current_id` from `start_id` to `end_id`:
        a.  **Extract Context**:
            - **Option A (Preferred): Use Script with File Output**:
                - Run the following command. **CRITICAL**: Use the `--output` argument to avoid terminal encoding issues.
                  ```powershell
                  python .\prepare_analysis_context.py --id <current_id> --report-path ".\HTML File Categorization Report.md" --root-dir "." --output temp_context_<current_id>.json
                  ```
                - Read the generated `temp_context_<current_id>.json` file.
            - **Option B (Fallback): Direct Read**:
                - If the script fails or JSON is invalid, read `HTML File Categorization Report.md` to find the filename for the `<current_id>`.
                - Then, directly read the HTML file content using `view_file` or `cat`.

        b.  **Analyze Content**:
            - Review the content (from JSON `text_preview` or direct HTML read).
        ...
        c.  **Formulate Plan**:
            - If a need for an image is found:
                ...
                - **Global Uniqueness Check (ABSOLUTE RULE)**:
                    - **Step 1: Check Existing Images in Current HTML**:
                        - Parse the HTML content to find ALL `<img src="...">` tags.
                        - Extract the filenames from the `src` attributes (e.g., `cnt_foo_bar.png`).
                        - **CRITICAL CONSTRAINT**: The proposed filename must NOT match ANY filename already present in the HTML file. 
                        - **Action on Match**: If the exact filename (e.g., `cnt_foo.png`) is ALREADY present in the HTML file's `<img>` tags, **SKIP ONLY this specific image proposal**. Do NOT append a suffix. Assume the image is already implemented. Do NOT stop analyzing the rest of the file.
                    ...
                    - **Resolution**:
                        - If a match is found in **Step 1 (HTML Check)**: **DISCARD** this specific image proposal entirely (do not add to plan).
        ...
        e.  **Modify HTML File**:
            - **Action**: Insert the image tag into the source HTML file.
            - **Tag**: `<p><img src="<relative_link>"></p>`
            - **Method**:
                ...
                - **Constraint**: Do NOT use `div`, `class`, `attr`, or `edge` attributes. Just a simple `<p>` wrapping an `<img>`.
        ...
        f.  **Verify Consistency**:
            ...
            - **Check**: Read the HTML file again and search for `<img src="<relative_link>">`.
            ...
            - **Final Check**: Run `grep` or `findstr` to confirm the presence of the new image filename in the HTML file before finishing. If it's not there, you have FAILED.
