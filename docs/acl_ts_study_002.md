...
![イメージ：内側・外側・境界の地図](./picture/acl_ts_study_002_system_map.png)

```mermaid
flowchart TD
    subgraph Internal ["内側 (自分の世界) 📘✨"]
        Domain["Domain (ルール)"]
        UseCase["UseCase (やりたい事)"]
    end
    
    subgraph ACL ["ACL (腐敗防止層) 🧼🛡️"]
        Trans["Translator (翻訳)🔁"]
        Verify["パース/検証🧽✅"]
        Error["エラー翻訳🧊🔥"]
    end
    
    subgraph External ["外側 (相手の世界) 🌍🌀"]
        StudentAPI["学生情報API"]
        PaymentAPI["決済API"]
    end
...
flowchart TD
    Root["学食ポイントアプリ (Root)"]
    Domain["domain/ (ルールと言葉) 💎"]
    App["application/ (ユースケース) 🧠"]
    Infra["infrastructure/ (実装) 🔌"]
    
    subgraph Details ["Infrastructure Inner"]
        ACL["acl/ (翻訳・防波堤) 🧼🛡️"]
        Ext["external/ (API通信) 🌍"]
    end
