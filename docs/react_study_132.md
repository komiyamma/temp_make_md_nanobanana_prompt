...
![Hook Indices](./picture/react_study_132_hook_indices.png)
...
```mermaid
flowchart TD
  A[レンダー開始] --> B[1個目のフックを呼ぶ]
  B --> C[2個目のフックを呼ぶ]
  C --> D[3個目のフックを呼ぶ]
  D --> E[Reactが「順番」で状態をひも付け]
```
...
![フックルールのイメージ](./picture/react_study_132_hook_rules.png)

```mermaid
flowchart TB
  R[フックのルール] --> A[トップレベルで呼ぶ]
  R --> B[条件分岐・ループ・ネスト関数の中で呼ばない]
  R --> C[Reactコンポーネント or カスタムフックの中で呼ぶ]
```
