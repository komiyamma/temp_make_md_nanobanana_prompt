...
![イメージ：エラー翻訳辞書（ステータスコードを意味に変換）](./picture/acl_ts_study_017_error_mapping_table.png)

```mermaid
flowchart LR
    subgraph Status ["HTTP Status / Response 🌩️"]
        S1["429"]
        S2["503"]
        S3["404"]
        S4["401"]
    ...
    subgraph Meaning ["Domain Meaning 📘"]
        M1["RateLimited"]
        M2["TemporaryUnavailable"]
        M3["NotFound"]
        M4["Unauthorized"]
    end
...
![イメージ：リトライ判定の信号機](./picture/acl_ts_study_017_retry_judge.png)

```mermaid
flowchart TD
    Issue["エラー発生 🌩️"] --> Q1{"一時的？ (Timeout/5xx)🌦️"}
    Q1 -- No --> Fail["リトライ不可 ❌"]
    Q1 -- Yes --> Q2{"冪等 (安全)？ (GET等)🧷"}
    Q2 -- No --> Fail
    Q2 -- Yes --> Q3{"上限内 (回数/時間)？ 🧯"}
    Q3 -- No --> Fail
    Q3 -- Yes --> Retry["リトライ実行 🔁✅"]
...
```mermaid
flowchart LR
    A1["1回目"] -->|Wait: 0~200ms| A2["2回目"]
    A2 -->|Wait: 0~400ms| A3["3回目"]
    A3 -->|Wait: 0~800ms| A4["4回目"]
    
    Note["指数バックオフ + ジッター 📈🎲<br/>(分散して再崩壊を防ぐ)"]
```
