...
```mermaid
graph TD
    subgraph Envelope ["Event Envelope (外側)"]
        Type["Event Type (名前)"]
        
        subgraph Payload ["Payload (中身 🍱)"]
            Fact["起きた事実そのもの<br/>(Sku, Qty, etc.)"]
        end
        
        subgraph Meta ["Metadata (ラベル 🏷️)"]
            Track["運用/追跡用の情報<br/>(EventId, Timestamp, etc.)"]
        end
    end
```
...
```mermaid
flowchart LR
    subgraph Correlation ["CorrelationId (ひと繋がりの文脈)"]
        direction LR
    ...
    Note1(A が B を引き起こし<br/>B が C を引き起こした)
```
