...
```mermaid
flowchart LR
  A[PCで開発🧑‍💻] --> B[GitHubへpush🐙]
  B --> C{デプロイ先を選ぶ🤔}
  C -->|Vercel| D[Vercelが自動ビルド⚙️]
  C -->|Cloudflare Pages| E[Pagesが自動ビルド⚙️]
  D --> F[distを公開📦]
  E --> G[distを公開📦]
  F --> H[URLで見れる🎊]
  G --> H
...
![デプロイのイメージ](./picture/react_study_160_deploy.png)
...
![SPA 404 Rewrite Rule](./picture/react_study_160_spa_404_rewrite.png)
...
sequenceDiagram
  ...
  participant H as Hosting(静的)
  U->>H: GET /users/123
  alt 対策なし
    H-->>U: 404 Not Found
  else 対策あり
    H-->>U: index.html を返す(200)
    U->>U: React Routerが /users/123 を表示✨
  end
```
...
## 環境変数（ちょい大人の話）🔐✨
![Environment Variables Security](./picture/react_study_160_env_variables.png)
