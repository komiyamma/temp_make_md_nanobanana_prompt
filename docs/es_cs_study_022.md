...
![競合のイメージ](./picture/es_cs_study_022_concurrency.png)
...
sequenceDiagram
    ...
    A->>S: 1. Load: 残高100円 (v0)
    B->>S: 2. Load: 残高100円 (v1 ではなく v0)
    Note over A,B: 二人とも「残高100円」だと思い込む
    A->>A: 3. Decide: 80円引ける！📜
    B->>B: 4. Decide: 80円引ける！📜
    A->>S: 5. Append: 80円引いた (v1)
    B->>S: 6. Append: 80円引いた (v2) ✅ 成功してしまう！
    Note over S: 最終残高は -60円 !! 😱💥
```
...
```mermaid
graph TD
    subgraph Client ["クライアント (判断)"]
        Load["1. Load (ver=10)"]
        Decide["2. Decide (ver=10 ベース)"]
    end
    
    subgraph Server ["EventStore (保存)"]
        Check{"3. 保存時にチェック <br/>(期待=10, 現在=10 ?)"}
        Success["4. OK: 保存 (ver=11)"]
        Fail["5. NG: 競合エラー ⚔️"]
    end
    
    Load -- "読み取り" --- Server
    Decide -- "AppendAsync(expected=10)" --> Check
    Check -- "一致" --> Success
    Check -- "不一致" --> Fail
```
