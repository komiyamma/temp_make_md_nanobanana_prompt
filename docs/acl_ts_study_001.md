...
![イメージ：防波堤と翻訳係](./picture/acl_ts_study_001_breakwater.png)

```mermaid
flowchart LR
    subgraph External ["外部 (相手の都合) 🌩️"]
        API["External API / DTO"]
    end
    
    subgraph ACL ["ACL (境界の門番) 🧱🛡️"]
        Trans["Translator (翻訳)"]
    end
    
    subgraph Internal ["アプリの内側 (自分のルール) 📘✨"]
        Model["Internal Model / Logic"]
    end
