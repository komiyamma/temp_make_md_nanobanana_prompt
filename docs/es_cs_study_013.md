...
graph LR
    ...
    ES -- "ReadStreamAsync 🔎" --> User
    
    subgraph ES_Inside [EventStore 内部]
        S1[(Stream A)]
        S2[(Stream B)]
    end
...
![EventStoreの中身](./picture/es_cs_study_013_storage.png)

```mermaid
graph TD
    subgraph Storage ["InMemory Storage (ConcurrentDictionary)"]
        K1["Key: 'cart-001'"] --- V1["List&lt;StoredEvent&gt; (Thread-Unsafe)"]
        K2["Key: 'cart-002'"] --- V2["List&lt;StoredEvent&gt; (Thread-Unsafe)"]
    end
    
    subgraph Locks ["Locks (ConcurrentDictionary)"]
        L1["Lock: 'cart-001'"]
        L2["Lock: 'cart-002'"]
    end
