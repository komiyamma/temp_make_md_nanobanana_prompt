...
![SQLiteスキーマ](./picture/es_cs_study_033_sqlite_schema.png)
...
classDiagram
    ...
    note for events "PRIMARY KEY(stream_id, version)"
```
...
![SQLiteの並行処理（WAL）](./picture/es_cs_study_033_sqlite_locking.png)
...
```mermaid
flowchart TD
    Start([AppendAsync 開始]) --> TX[1. トランザクション開始 🛡️]
    TX --> GetVer["2. 現在の MAX(version) を取得"]
    GetVer --> Check{3. 現在 == expected?}
    
    Check -- No --> Conflict[4. 競合例外をスロー 🛑]
    Check -- Yes --> Loop[5. 各イベントを INSERT]
    
    Loop --> Commit[6. コミット 🎉]
    Commit --> End([完了])
    Conflict --> Rollback[7. ロールバック]
    Rollback --> End
