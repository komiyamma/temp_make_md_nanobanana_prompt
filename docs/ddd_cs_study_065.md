...
![トランザクション管理](./picture/ddd_cs_study_065_transaction.png)
...
この2つが **セットで必ず同時に成立**してほしいですよね？
片方だけ成功すると、次みたいな地獄が起きます…😇🔥
...
* **複数集約を1回で更新**したくなるときは、設計が難しくなるゾーン⚠️

でも！現実は「複数集約を同時に更新したい」場面が普通にあります😊
だからこの章では **“迷わないための判断ルール”**を作ります🎯
...
* **Outbox（送信箱）パターン**で「あとで確実に送る」📮✨ ([Microsoft Learn][2])
...
EF Core では `DbContext.Database` からトランザクションを開始できます。 ([Microsoft Learn][1])
**ポイントは「ユースケース（アプリケーションサービス）で囲う」**ことです✨
ドメイン層にトランザクションを持ち込まないのがキレイです🧼
...
> * 最後にまとめて `SaveChanges` してコミット🎉
 ...
 sequenceDiagram
    ...
    participant Inv as Inventory集約
    
    App->>DB: Begin Transaction 🎬
    
    App->>Order: Create Order 🆕
    App->>Inv: Reserve Stock 📦
    
    rect rgb(240, 240, 240)
        Note over App, Inv: ここまでメモリ上の変化
    end
    
    App->>DB: SaveChanges (SQL送信)
    App->>DB: Commit! ✅
    
    Note over DB: ここで初めて確定！
 ```
...
* トランザクションの開始・終了が **ユースケース1つに対応**してる ✅
...
➡️ 外部通信は **基本「コミット後」**に寄せましょう😊
どうしても「確実に送る」が必要なら **Outbox** へ📮✨（後述）

### ❷ DbContextを並列で使う（Task.WhenAllとか）🏃‍♀️🏃‍♀️💥

同じ `DbContext` で並列実行は危険です⚠️
EF Core は同一 `DbContext` の並列操作をサポートしないので、基本は「すぐawait」がお作法です。 ([Microsoft Learn][3])

---

## もう一歩：TransactionScope はいつ使う？🧵🤏

`TransactionScope` は「スコープに入ってる処理をまとめてトランザクションにする」道具です。
ただし **async/awaitと一緒に使うなら設定が必須**で、`TransactionScopeAsyncFlowOption.Enabled` が必要です。 ([Microsoft Learn][4])
...
でも初心者のうちは、まずは **EF Coreの BeginTransactionAsync** をおすすめします😊
（TransactionScopeは状況によって分散トランザクション寄りの話になって急に難しくなるので…😵‍💫）
...
* DB更新と一緒に **Outboxテーブルに“送る予定”を保存**する（同じトランザクション）
* コミット後、別プロセス/バックグラウンドがOutboxを読んで送信する

この考え方が **Transactional Outbox** です📮✨ ([Microsoft Learn][2])
...
* 同一DB内は `BeginTransactionAsync` が基本で安心 ([Microsoft Learn][1])
* 外部通信が混ざるなら Outbox で事故を減らす📮✨ ([Microsoft Learn][2])

---

次の第66章（CQRS超入門）に行く前に、もしよければ✨
「いま作りたいアプリの“複数集約更新っぽい処理”」を1つ挙げてくれたら、**この章のルールで最短ルートの設計**に落とし込みますよ😊🧠💖
