...
```mermaid
graph TD
    Client[クライアント] -- "(1) 書き込み" --> Leader[Leader 👑]
    Leader -- "(2) 変更ログ送信" --> Follower1[Follower 🪞]
    Leader -- "(2) 変更ログ送信" --> Follower2[Follower 🪞]
    
    Client -. "(3) 読み取り" .-> Leader
    Client -. "(3) 読み取り" .-> Follower1
    Client -. "(3) 読み取り" .-> Follower2
...
![最終的整合性のイメージ（時計のズレ）](./picture/cap_ts_study_013_eventual_clocks.png)
