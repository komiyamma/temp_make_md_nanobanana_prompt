# 第05章：題材紹介：学内アプリ（例）と外部APIの“クセ”を確認 🎓🧾
...
![イメージ：外部APIのクセたち（データモンスター）](./picture/acl_ts_study_005_kuse_collection.png)
...
![イメージ：外部APIのクセを可視化する](./picture/acl_ts_study_005_kuse_ledger.png)
...
```mermaid
flowchart LR
    A["外部データ (Quirky Data) 🌀"] --> B{ACL 分類分け 🗃️}
    B --> C["命名・コード変換 🧾"]
    B --> D["パース・整形 🗓️"]
    B --> E["欠損・単位統一 🧼"]
    B --> F["エラー翻訳 🧊"]
    C & D & E & F --> G["ドメインモデル (Clean Model) 💎"]
```
