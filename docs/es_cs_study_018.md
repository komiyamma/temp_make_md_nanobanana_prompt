...
sequenceDiagram
    ...
    participant H as UseCase (Handler)
    ...
    H->>S: 1. Load: 読込 (新規は空) 📚
    S-->>H: 空のリスト
    H->>A: 2. Rehydrate: 初期状態で作成 🧠
    H->>A: 3. Decide: 作成イベント生成 🆕
    A-->>H: CartCreated 📜
    H->>S: 4. Append: 保存 (v0 -> v1) 📦
    S-->>H: 完了
    H-->>U: 成功通知 🎉
```
...
```mermaid
flowchart TD
    Start([DecideCreate 実行]) --> IsCreated{すでに作成済み?}
    IsCreated -- Yes --> Fail["空リストを返す <br/>(or 次章でエラー) 🛑"]
    IsCreated -- No --> Success[CartCreated イベントを生成 🆕]
