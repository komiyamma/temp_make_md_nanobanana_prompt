...
![バージョンチェックのルール（v1/v2/v3）](./picture/cap_ts_study_024_version_check.png)

```mermaid
flowchart TD
    Start([新イベント到着]) --> Q{version 比較}
    Q -- "v(新) <= v(現)" --> Stale[しらっと無視 🧊]
    Q -- "v(新) == v(現) + 1" --> Apply[反映！✅]
    Q -- "v(新) > v(現) + 1" --> Gap[抜けあり！再取得へ 📥]
