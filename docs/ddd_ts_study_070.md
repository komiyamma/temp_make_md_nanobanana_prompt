...
sequenceDiagram
    autonumber
    participant Client
    participant App as App Context (Bootstrap)
    participant Repo as InMemory Repo
    participant DB as Store (Map)

    Note over Client, DB: 1. Place Order 🎬
    Client->>App: placeOrder.execute(cmd)
    App->>Repo: save(Draft Order)
    Repo->>DB: set(id, order)
    
    Note over Client, DB: 2. Pay Order 💳
    Client->>App: payOrder.execute(cmd)
    App->>Repo: findById(id)
    Repo->>DB: get(id)
    App->>App: order.pay() 🔒
    App->>Repo: save(Paid Order)
    
    Note over Client, DB: 3. Fulfill Order ☕
    Client->>App: fulfillOrder.execute(cmd)
    App->>Repo: findById(id)
    App->>App: order.fulfill() 🔒
    App->>Repo: save(Fulfilled Order)
    
    Note over Client, DB: 4. Get Order 🔎
    Client->>App: getOrder.execute(query)
    App->>Repo: findViewById(id)
    Repo-->>Client: View DTO 📦
```
