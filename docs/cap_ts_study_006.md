...
![CAP定理の三角形](./picture/cap_ts_study_004_cap_triangle.png)
...
```mermaid
flowchart TD
    P{"分断(P)発生！"} -- 接続不可 --> C["C寄り: 止める"]
    P -- 接続不可 --> A["A寄り: 進める"]
    
    C --> C1["「正しくないなら売らない」<br/>(エラーを返す)"]
    A --> A1["「ズレてもいいから売る」<br/>(後で整合させる)"]
    
    style C fill:#fdd,stroke:#c33
    style A fill:#dfd,stroke:#3c3
```

![CAPトレードオフ](./picture/cap_ts_study_006_cap_tradeoff.png)
