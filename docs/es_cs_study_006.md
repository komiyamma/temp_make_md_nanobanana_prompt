...
![CRUDの限界](./picture/es_cs_study_006_crud_limit.png)
...
```mermaid
graph TD
    subgraph Update_CRUD ["CRUD (Overwrite)"]
        D1[旧: タイトルA] -- 上書き --> D2[新: タイトルB]
        D1 -. 消失 .-> Trash[ゴミ箱]
    end

    subgraph Update_ES ["ES (Event Sourcing)"]
        E1[追加: タイトルA] --> E2[変更: タイトルB]
    end
```
