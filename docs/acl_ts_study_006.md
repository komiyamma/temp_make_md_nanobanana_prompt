...
```mermaid
sequenceDiagram
    participant Ext as 外部 API 🌩️
    participant App as アプリ (内側) 💀
    
    Ext->>App: pt を points に改名 🔁
    Note over App: 型定義 (Dto) を直で使っている
    App->>App: dto.pt が undefined 😱
    App->>App: Number(undefined) = NaN
    Note over App: 判定ロジックが全部バグる 💥
```
...
![イメージ：外部DTOによる内側の侵食](./picture/acl_ts_study_006_contamination.png)
