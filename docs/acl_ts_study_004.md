...
![イメージ：学園祭の屋台（役割の分離）](./picture/acl_ts_study_004_soc_kitchen.png)
...
![イメージ：入国審査（境界でのチェック）](./picture/acl_ts_study_004_internal_external.png)
...
```mermaid
flowchart TD
    Ext["外部API 🌩️<br/>(相手の都合)"]
    ACL["ACL (通訳＋防波堤) 🛡️<br/>(境界)"]
    Int["内側: ドメイン 🧠<br/>(自分のルール)"]
    
    Ext -->|"JSON/謎コード<br/>(External DTO)"| ACL
    ACL -->|"ドメイン型<br/>(Internal Model)"| Int
