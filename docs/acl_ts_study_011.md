...
![イメージ：マッピング（変換）の基本ステップ](./picture/acl_ts_study_011_mapping_flow.png)

```mermaid
flowchart LR
    In["DTO 受信 📥<br/>(Raw Data)"] --> Clean["整形・パース 🧽<br/>(Trim, Number化)"]
    Clean --> Build["ドメイン型作成 📤<br/>(VO/Entity 組み立て)"]
    Build --> Out["内側へ 🌸<br/>(Clean Data)"]
```
...
```mermaid
flowchart TD
    DTO["StudentDirectoryStudentDto 🧾"]
    
    subgraph Mappers ["Mappers 🔁"]
        M1["toStudentType (grade_cd)"]
        M2["toPoints (string to number)"]
    end
