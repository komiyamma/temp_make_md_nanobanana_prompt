...
sequenceDiagram
    ...
    U->>S: 1. コマンド実行 (AddItem)
    S->>E: 2. Append (イベント保存)
    E-->>S: 3. 保存された EventEnvelope 群
    S->>P: 4. Project (同期呼び出し) ⚡
    P->>V: 5. Upsert (読みモデル更新)
    P-->>S: 6. 完了
    S-->>U: 7. 成功レスポンス
```
...
![チェックポイントの仕組み](./picture/es_cs_study_029_checkpoint.png)

```mermaid
flowchart TD
    In([イベント v10 受信]) --> GetCP["1. checkpoint (最後に処理した版) を取得"]
    GetCP --> Compare{2. 10 <= checkpoint ?}
    
    Compare -- Yes --> Skip[3. 処理済みなのでスキップ ⏩]
    Compare -- No --> Apply[4. 読みモデルに反映 🍱]
    
    Apply --> SaveCP[5. checkpoint を 10 に更新 🔖]
    SaveCP --> End([完了])
    Skip --> End
