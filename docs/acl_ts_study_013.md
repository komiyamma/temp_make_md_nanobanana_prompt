...
```mermaid
flowchart LR
    Raw["外部コード 👻<br/>('1', '2', '9')"] -->|ACL 翻訳| Domain["内側の型 📘<br/>(Undergrad, Grad, Other)"]
    
    style Domain fill:#E6F7FF,stroke:#3399FF
```
...
![イメージ：列挙型のマッピングと未知の受皿](./picture/acl_ts_study_013_enum_tray.png)

```mermaid
flowchart TD
    Code["外部から来たコード 🔤"] --> Valid{既知のコード？}
    Valid -- Yes --> Map["マッピング実行 ✅"]
    Valid -- No --> Policy{対応方針は？🚦}
    
    Policy -- A --> Error["即エラー (方針A) 💥"]
    Policy -- B --> Round["Other/Unknown に丸める (方針B) 🧺"]
    Policy -- C --> Quarantine["隔離・要調査 (方針C) 🧊"]
```
...
```mermaid
graph TD
    Domain["Domain Model 📘<br/>(Core Concept)"]
    UI["UI Layer 🎨<br/>(Display Labels)"]
    
    Domain -->|Enum Value| UI
    UI -.->|'UNDERGRAD'| Label["'学部生'"]
    UI -.->|'GRAD'| Label2["'院生'"]
```
