...
```mermaid
flowchart LR
    subgraph Data ["テストデータ Table 🍣"]
        C1["未知コード"]
        C2["null 混入"]
        C3["型違い"]
    end
    
    Data --> Runner["Vitest it.each"]
    Runner -->|Call| Mapper["mapStudent"]
    Mapper -->|Expect| Assert["toThrow ✅🛑"]
```
...
![イメージ：外部仕様変更の自動検知シグナル](./picture/acl_ts_study_019_change_sensor.png)

```mermaid
flowchart TD
    subgraph Samples ["外部レスポンス標本 📦"]
        F1["fixtures/success.json"]
        F2["fixtures/error.json"]
    end
    
    Samples --> Mapper["ACL Mapper 🧱"]
    Mapper --> Success{成功？}
    
    Success -- Yes --> OK["テストパス ✅"]
    Success -- No --> Alert["警告！外部仕様変更 🚨🚨"]
```
