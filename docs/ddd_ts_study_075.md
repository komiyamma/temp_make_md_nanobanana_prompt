...
```mermaid
flowchart TD
    Start([Factoryが必要？]) --> Simple{"生成はシンプル？<br/>(引数だけで完結)"}
    
    Simple -- Yes --> Static["Static Factory 🧊<br/>Order.create(...)"]
    Simple -- No --> Deps{"外部依存がある？<br/>(ID生成, Clock, Repository...)"}
    
    Deps -- Yes --> Class["Factory Class 🏭<br/>new OrderFactory(...)"]
    Deps -- No --> Complex{生成パターンが多い？\n手順が複雑？}
    
    Complex -- Yes --> Class
    Complex -- No --> Static
    
    style Static fill:#e6fffa
    style Class fill:#fff0f0
```
