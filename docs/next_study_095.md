...
```mermaid
flowchart TD
  A["データ取得 fetch"] --> B{"結果どうだった？"}
  B -->|"通信できず例外"| N["通信エラー📡: 再試行/案内"]
  B -->|"通信はできたが res.ok=false"| H["APIエラー🚦: 404/500など<br/>＝通信じゃなく「応答エラー」"]
  B -->|"取得後の処理で例外"| C["コードエラー🐛: バグ修正が本命"]
  B -->|"原因不明/レア"| U["想定外🎲: 退避UI + ログ"]
```
