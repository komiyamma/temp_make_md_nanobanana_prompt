...
graph TD
    subgraph Bad ["悪い例 ❌ (癒着)"]
        Agg1["集約 🧺"] -->|直呼び| DB1[("DB / API 🔌")]
    end
    
    subgraph Good ["良い例 ✅ (分離)"]
        direction LR
        S["App Service 📦"] -->|1. Data| DB2[("DB / API 🔌")]
        S -->|2. Logic| Agg2["集約 🧺"]
    end
...
sequenceDiagram
    ...
    participant DB as Repository / API 🔌
    participant A as 集約 (Domain) 🧠
    
    S->>DB: 1. 外の世界からデータを集める
    DB-->>S: 
    S->>A: 2. 集まったデータを渡して判断・更新を頼む
    Note over A: 純粋なルールチェック🔒
    A-->>S: 
    S->>DB: 3. 変わった結果を保存・実行する
```
