...
![楽観ロックの仕組み](./picture/es_cs_study_023_optimistic_lock.png)
...
sequenceDiagram
    ...
    A->>S: 3. Append (expected=10)
    Note over S: 現在=10 なので OK ✅
    S-->>A: 成功 (New v11)
    
    B->>S: 4. Append (expected=10)
    Note over S: 現在=11 なので NG !! 🛑
    S-->>B: 競合エラー (Conflict)
    Note over B: リトライなどへ 🔁
```
...
public sealed class InMemoryEventStore
{
    ...
    public int ReadStreamCount(string streamId)
    {
        return _streams.TryGetValue(streamId, out var list) ? list.Count : 0;
    }
}
```
...
```mermaid
flowchart TD
    Start([Append 呼び出し]) --> Lock["1. lock (排他制御) 開始 🔐"]
    Lock --> GetStream[2. 対象ストリームを取得]
    GetStream --> CheckVer{"3. 現在の版 == <br/>期待する版?"}
    
    CheckVer -- No --> Conflict[4. 競合例外をスロー 🛑]
    CheckVer -- Yes --> Loop[5. 各新イベントに <br/>連番を振って追加 🧾]
    
    Conflict --> Unlock[6. lock 解放 🔓]
    Loop --> Unlock
    Unlock --> End([終了])
