...
```mermaid
flowchart LR
    External["External World 🌍<br/>(UI / API Client)"]
    
    subgraph AppLayer [Application Layer 🎬]
        InputDTO["Input DTO 📥<br/>(Primitives)"]
        OutputDTO["Output DTO 📤<br/>(Primitives)"]
        AppService["App Service 🧑‍🍳"]
        Mapper["Mapper/Translator 🔁"]
    end
