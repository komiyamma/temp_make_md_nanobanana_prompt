...
```mermaid
flowchart TB
  U["ユーザー操作🧑‍💻"] --> UI["UI: ボタン表示/非表示😺"]
  U --> RQ["リクエスト送信📨"]

  subgraph "本物の守り🛡️（サーバー側）"
    MW["① Middleware: 入口で止める🧤"]
    SC["② Server Component/layout: 本体で確認🧊"]
    API["③ Route Handler/Server Actions: 操作を守る🚪"]
  end

  RQ --> MW
  MW -->|"OK"| SC
  MW -->|"NG"| RED["redirect/login など🚦"]
  SC --> API
  API -->|"OK"| OK["処理成功🎉"]
  API -->|"NG"| DENY["403/redirect/エラー表示🙅‍♀️"]
```
