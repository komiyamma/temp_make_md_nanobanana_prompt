...
![State Preservation](./picture/es_cs_study_002_state_preservation.png)
...
![Event Preservation](./picture/es_cs_study_002_event_preservation.png)
...
![3ステップ学習法](./picture/es_cs_study_002_study_loop.png)

```mermaid
flowchart TD
    subgraph Traditional ["状態保存 (Traditional)"]
        TB[(残高テーブル)]
        TB --- B1[残高: 10,000円]
    end

    subgraph ES ["イベントソーシング (ES)"]
        EL{イベントの列}
        EL --> E1[1. 入金 +5,000円]
        EL --> E2[2. 出金 -2,000円]
        EL --> E3[3. 入金 +7,000円]
    ...
    Traditional -. "なぜこうなった？" .-> ES
    ES -- "計算して復元" --> Traditional
```
...
![Audit Log](./picture/es_cs_study_002_audit_log.png)

「いつ・誰が・何をした」が自然に残るよ。監査ログ（監査証跡）っぽいのが**標準装備**になる感じ🕵️‍♀️✅

## 2) “タイムトラベル”できる🕰️🔁

![Time Travel](./picture/es_cs_study_002_time_travel.png)
...
![Household Account Example](./picture/es_cs_study_002_household_account.png)
...
![Apply Flow](./picture/es_cs_study_002_apply_flow.png)
...
sequenceDiagram
    ...
    participant E1 as Deposited(+100)
    participant E2 as Withdrawn(-30)
    participant E3 as Deposited(+50)
    participant R as Final State

    Note over S: Balance: 0
    S->>E1: Apply
    Note over E1: Balance: 100
    E1->>E2: Apply
    Note over E2: Balance: 70
    E2->>E3: Apply
    Note over E3: Balance: 120
    E3->>R: Rehydrate Complete
```
