...
```mermaid
flowchart TD
    Start([FulfillOrder Request]) --> Find[Find Order]
    Find --> CheckStatus{Current Status?}
    
    CheckStatus -- Draft/Confirmed --> ErrorPre[Error: Not Paid Yet 🚫]
    CheckStatus -- Cancelled --> ErrorCancel[Error: Cancelled 🚫]
    CheckStatus -- Fulfilled --> ErrorDone[Error: Already Fulfilled 🔁]
    
    CheckStatus -- Paid 💳 --> OK[OK to Fulfill ✅]
    
    OK --> DomainOp["Order.fulfill()"]
    DomainOp --> UpdateStatus[Status = FULFILLED]
    UpdateStatus --> Save[Save Order]
    Save --> End([Success 🎉])
    
    style ErrorPre fill:#ffcccc
    style ErrorCancel fill:#ffcccc
    style ErrorDone fill:#ffffcc
