...
この章が終わると、**DB/HTTP/外部APIなどの「I/O層で起きた例外」を、毎回ブレずに Result に変換**できるようになるよ😊✨
そして、**ユーザー表示**と**ログ**をキレイに分けられるようになる！🫶🧾
...
2. **例外→Result変換は“1か所に集める”**📍
   → いろんな所で `catch` してバラバラ変換すると、地獄化する😇

3. **ユーザー向け文言に、例外メッセージを使わない**🚫🧨
   → 例外メッセージは内部情報・英語・不安を煽る、になりがち。
...
5. **タイムアウト/一時的ネット不安定は “retryable: true” が基本**⏳🔁
   （ただし回数・間隔は別途ガード！）

6. **キャンセルは “障害” ではなく “操作結果”**🛑🙂
   → ログは Error にしないことが多い（Info/Debug寄り）

7. **ログには「例外＋相関ID＋外部依存名＋操作名」を残す**🔎🧵
   → 後から追えるかが全て！
...
最近の .NET では、**タイムアウト時に TaskCanceledException が飛び、InnerException が TimeoutException になる**ケースが多いよ（見分けに使える）🧠✨ ([Microsoft Learn][1])
...
`EnsureSuccessStatusCode()` を使うと `HttpRequestException` が飛ぶんだけど、最近の .NET では **StatusCode プロパティ**で判定できるよ😊 ([Microsoft Learn][1])
...
`AddStandardResilienceHandler` が用意されてるのがポイント✨ ([Microsoft Learn][2])

> ただし！
> **リトライしてもダメだった最後の失敗**は、今回の章どおり **Resultに変換**して上へ渡すのがキレイ🎁✨
...
SqlClient には **Configurable Retry Logic** があって、transient エラー番号などを使って制御できるよ🧠 ([Microsoft Learn][4])

（初心者向けの結論）
➡️ **“リトライで救う” と “最後はResult変換” をセット**で覚えるのが最強✨
...
> ここでのキモはね👇
> **「例外の種類」→「失敗の意味（コード/再試行/行動）」に翻訳して返す**こと🎁✨
...
2. `UserMessage` を **やさしい日本語**で作る💬
3. `DetailForLog` に **例外全文**を入れる🧯
4. `Retryable` と `ActionHint` を決める🔁🫶
...
* インフラ例外は **Resultのインフラエラーに翻訳**しよう🎁
* 変換は **1か所に集約**がラク📍
* **timeout/503/ネット不安定は retryable** の考え方が基本🔁
* ログは **追跡できる項目セット**で構造化して残す🔎🧵 ([Microsoft Learn][5])
