# 第15章：外部データを信用しない② バリデーション（検証）の設計 ✅🧪
...
![イメージ：バリデーションという検品所](./picture/acl_ts_study_015_validation_checkpoint.png)

```mermaid
flowchart LR
    Ext["外部 DTO 📦<br/>(他人の荷物)"] -->|検品 🔍✅| ACL["ACL 🚧"]
    ACL -->|安全なデータ| Int["内側 🌸<br/>(自分たちの部屋)"]
    ACL -- NG --> Err["エラー / 隔離 🚨"]
```
...
![イメージ：バリデーション失敗時の3つの進路](./picture/acl_ts_study_015_validation_3_ways.png)

```mermaid
flowchart TD
    Issue["検証失敗 🚨"] --> Policy{対応方針は？🚦}
    
    Policy -- ① --> Drop["落とす (Stop) ❌<br/>重要データ向き"]
    Policy -- ② --> Replace["代替値 (Default) 🧸<br/>表示用・非重要"]
    Policy -- ③ --> Quarantine["隔離 (Quarantine) 🧊📦<br/>要調査・観測"]
```
...
```mermaid
flowchart LR
    Start["開始"] --> Parse["1. パース 🧽<br/>(整える)"]
    Parse --> Validate["2. バリデーション ✅<br/>(確かめる)"]
    Validate --> Build["3. モデル化 📘<br/>(型にする)"]
    Build --> End["完了"]
```
