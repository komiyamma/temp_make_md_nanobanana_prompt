...
![イメージ：Ports & Adaptersの構造](./picture/acl_ts_study_010_ports_adapters.png)
...
![イメージ：Composition Root（パーツを組み立てる場所）](./picture/acl_ts_study_010_composition_root.png)

```mermaid
flowchart TD
    Main["Composition Root (Main) 🧩"]
    Adap1["StudentApiAdapter"]
    Adap2["PaymentApiAdapter"]
    UC["ChargeLunchPointsUseCase"]
    
    Main -->|new| Adap1
    Main -->|new| Adap2
    Main -->|Inject| UC
    
    UC -->|Uses| Port1["StudentDirectoryPort (IF)"]
    UC -->|Uses| Port2["PaymentPort (IF)"]
    
    Adap1 -- implements --> Port1
    Adap2 -- implements --> Port2
```
...
```mermaid
flowchart TB
    subgraph Test_Standard ["既存のテスト (結合寄り) 🐌"]
        App1["App"] --> Port1["Port"]
        Adap1["Real Adapter"] -- implements --> Port1
        Adap1 --> Net["Network / Actual API"]
    end
    
    subgraph Test_Fast ["Ports & Adaptersテスト ⚡"]
        App2["App"] --> Port2["Port"]
        Adap2["Fake / Mock Adapter"] -- implements --> Port2
        Adap2 --> Memory["In-Memory Data"]
    end
```
