...
![イメージ：マッピング関数の厳格なテスト](./picture/acl_ts_study_018_mapper_test.png)

```mermaid
flowchart LR
    DTO["外部 DTO 📦<br/>(変な値含む)"] -->|Test Target| Mapper["Mapper (翻訳係) 🔁"]
    Mapper -->|Verify Output| Res["Domain Object 💎<br/>(不変条件遵守)"]
...
```mermaid
flowchart TD
    Setup["1. セットアップ 🧁<br/>(makeStudentDto)"] --> Act["2. 実行 🔁<br/>(mapStudentDtoToStudent)"]
    Act --> Assert["3. 検証 ✅<br/>(expect)"]
    
    subgraph Verify ["検証内容"]
        V1["r.ok is true"]
        V2["id matches"]
        V3["points matches"]
    end
