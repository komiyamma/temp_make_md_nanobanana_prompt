...
![Decideパターンの型](./picture/es_cs_study_016_decide_pattern.png)
...
sequenceDiagram
    ...
    participant A as Aggregate 🛡️

    U->>H: Command 📮
    H->>S: 1. Load: 過去イベントを読込む 📚
    S-->>H: イベント列
    H->>A: 2. Rehydrate: 状態を復元 🧠
    H->>A: 3. Decide: 新イベントを決定 🛡️
    A-->>H: 新イベント 📜
    H->>S: 4. Append: 新イベントを保存 🧾
    S-->>H: 完了
    H-->>U: 成功レスポンス ✅
```
...
```mermaid
flowchart TD
    Start([Command 受信]) --> StreamID[1. ストリームIDを決定]
    StreamID --> Load[2. EventStore から読込]
    Load --> Rehydrate[3. Aggregate を復元]
    Rehydrate --> Decide[4. ビジネスルールで判定]
    Decide --> Envelope[5. メタデータで包む]
    Envelope --> Append[6. EventStore へ保存]
    Append --> End([完了通知])
```
