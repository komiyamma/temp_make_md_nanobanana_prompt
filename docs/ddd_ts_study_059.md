...
```mermaid
graph TD
    subgraph Healthy [✅ Healthy Aggregate]
        direction TB
        Root[Order Root] -->|Protect| Rules[Rules & Logic]
        Root -->|Manage| State[State]
    end

    subgraph Anemic ["❌ Anemic Model (Anti-Pattern)"]
        direction TB
        Service[OrderService] -->|Logic| DTO["Order (Data Only)"]
        DTO -->|Direct Access| Ext[External Code]
    end
    
    style Healthy fill:#e6fffa,stroke:#38b2ac
    style Anemic fill:#fff5f5,stroke:#fc8181
```
