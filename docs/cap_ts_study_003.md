...
![セットアップ](./picture/cap_ts_study_003_setup_robot.png)
...
```mermaid
graph LR
    User[お客さん] --> Order[注文 Receipt]
    Order -. 依頼 .-> Pay[決済 Payment]
    Order -. 予約 .-> Inv[在庫 Inventory]
    style Order fill:#f9f,stroke:#333
    style Pay fill:#bbf,stroke:#333
...
```mermaid
stateDiagram-v2
    [*] --> Pending: 注文受付(OrderPlaced)
    Pending --> Confirmed: 成功(Payment+Stock OK)
    Pending --> Rejected: 失敗(いずれかがNG)
    Confirmed --> [*]
    Rejected --> [*]

    state Pending {
        direction LR
        PaymentRequested --> PaymentAuthorized
        StockRequested --> StockReserved
    }
```
