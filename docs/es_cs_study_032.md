...
```mermaid
flowchart LR
    subgraph Truth [事実の記録]
        ES[(EventStore 📜)]
    end
    
    subgraph Rebuild [再構築プロセス]
        direction TB
        Reset[1. 古い View を全消し 🧹]
        Read[2. 全イベントを順に読込 📖]
        Apply[3. 1つずつ Apply 🔁]
        Read --> Apply
    end
    
    ES -- "ReadAll (Position順)" --> Read
    Apply -- "更新" --> NewV[新しい読みモデル 🍱]
    
    style Truth fill:#e8f5e9
    style Rebuild fill:#fff9c4
    style NewV fill:#e1f5fe
```
...
```mermaid
gantt
    title Position (全体の順) vs Version (ストリーム内の順)
    dateFormat  X
    axisFormat  %s
    ...
    section Cart-A (Version)
    v0 (pos=1) : 0, 1
    v1 (pos=3) : 2, 3

    section Cart-B (Version)
    v0 (pos=2) : 1, 2
    v1 (pos=4) : 3, 4
```
...
```mermaid
flowchart TD
    Start([Rebuild 開始]) --> Reset[1. Projection.ResetAsync]
    Reset --> ResetCP["2. Checkpoint.SaveAsync(0)"]
    ResetCP --> Read["3. EventStore.ReadAllAsync(pos, batch)"]
    
    Read --> HasData{データあり?}
    
    HasData -- Yes --> Apply[4. 1イベントずつ ApplyAsync]
    Apply --> SaveCP["5. Checkpoint.SaveAsync(pos)"]
    SaveCP --> Read
    
    HasData -- No --> End([再構築完了])
