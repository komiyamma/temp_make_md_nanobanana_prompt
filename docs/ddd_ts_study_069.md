...
```mermaid
flowchart TB
    subgraph Bad [❌ Bad: Rule Leakage 漏れ漏れ]
        direction TB
        App1[AppService A]
        App2[AppService B]
        Entity1[Anemic Order 🩸]
        
        App1 -- "if (status == PAID)" --> Entity1
        App1 -- "order.status = FULFILLED" --> Entity1
        
        App2 -- "if (status == PAID)" --> Entity1
        App2 -- "order.status = FULFILLED" --> Entity1
        
        note1[ルールがアプリ層に散らばる<br/>コピペ地獄・変更漏れ]
    end

    subgraph Good [✅ Good: Encapsulation カプセル化]
        direction TB
        App3[AppService A]
        App4[AppService B]
        Entity2[Rich Order 🏯]
        
        App3 -- "order.fulfill()" --> Entity2
        App4 -- "order.fulfill()" --> Entity2
        
        Entity2 -- "Check Status & Update" --> Entity2
        
        note2[ルールはドメインが守る<br/>アプリ層は呼ぶだけ！]
    end
    
    style Bad fill:#fff0f0,stroke:#f00
    style Good fill:#f0fff0,stroke:#090
```
