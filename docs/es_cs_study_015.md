...
![復元プロセス](./picture/es_cs_study_015_rehydration.png)
...
```mermaid
graph LR
    subgraph Stream ["Event Stream (過去の履歴)"]
        E0[v0: Created]
        E1[v1: Added]
        E2[v2: Added]
        E3[v3: Removed]
    ...
    Empty((空の状態)) --> A0 --> S1[状態更新] --> A1 --> S2[状態更新] --> A2 --> S3[状態更新] --> A3 --> Final((最新の状態))
...
```mermaid
flowchart TD
    Start([1. 空のインスタンスを作成]) --> Loop{2. イベントを1つ取り出す}
    Loop -- 全件終了 --> End([3. 組み立て完了した状態を返す])
    Loop -- 次のイベントあり --> Apply[4. Apply メソッドに渡す]
    Apply --> Loop
```
