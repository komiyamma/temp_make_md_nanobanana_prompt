...
![Upcastingのイメージ](./picture/es_cs_study_035_upcasting.png)

```mermaid
flowchart LR
    DB[(EventStore)] -- "1. v1 JSON 読込" --> Upcast{Upcaster 🧙‍♀️}
    Upcast -- "2. 足りない項目を補完 <br/>(Mapping)" --> Object["3. 最新形 (v2) オブジェクト"]
    
    Object --> Domain[Domain Logic / Projection]
    
    style DB fill:#f5f5f5
    style Upcast fill:#fff9c4
...
![冪等性の流れ](./picture/es_cs_study_035_idempotency_workflow.png)

```mermaid
flowchart TD
    Req([Request 受信]) --> Check{"1. idempotency_key <br/>は既にある?"}
    
    Check -- Yes --> GetRes["2. 保存済みの Response <br/>を即座に返す 📝"]
    Check -- No --> Reserve["3. 'processing' で予約 🏁"]
    
    Reserve --> Action["4. 本来の処理実行 <br/>(イベント追加等)"]
    Action --> Complete[5. 結果保存 & <br/>'completed' に更新 ✅]
    Complete --> Resp([Response 返却])
    GetRes --> Resp
    
    style GetRes fill:#e8f5e9
