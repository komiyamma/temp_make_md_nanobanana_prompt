...
![不変条件ガード](./picture/es_cs_study_009_invariant_guard.png)
...
```mermaid
graph TD
    User[ユーザー入力] --> Val[入力チェック<br/>Validation]
    Val -- OK --> UI[UI / API層]
    UI --> Domain[ドメイン層]
    Domain --> Inv{不変条件ガード<br/>Invariant}
    Inv -- 違反 --> Reject[拒否 🛑]
    Inv -- OK --> Event[イベント生成 📜]
```
...
```mermaid
flowchart TD
    Cmd[1. コマンド受信 📮] --> Rehyd[2. 状態復元 🔁<br/>Rehydrate]
    Rehyd --> Guard[3. 不変条件チェック 🧷<br/>Invariant Guard]
    Guard -->|違反| Error[4. 拒否 ❌]
    Guard -->|OK| Gen[5. イベント生成 📜]
    Gen --> Store[6. 保存 📚]
```
