...
```mermaid
sequenceDiagram
    participant U as ユーザー/フロント端 📱
    participant A as API (Command) 📮
    participant Q as キュー (Channel) 📬
    participant W as Worker (Background) 👷‍♀️
    participant V as View (ReadModel) 🍱

    U->>A: 1. コマンド送信
    A->>Q: 2. 仕事を積む (Enqueue)
    A-->>U: 3. 202 Accepted (OperationId) 📬
    
    Note over Q,W: 別スレッドで実行 ⏳
    Q->>W: 4. 仕事を取り出す (Dequeue)
    W->>V: 5. 投影実行 (Apply)
    V-->>W: 完了
    W->>W: 6. Status を done に更新 🔖
```
...
![非同期反映のフロー](./picture/es_cs_study_031_polling_flow.png)
...
sequenceDiagram
    ...
    U->>A: 1. Status 取得 (opId)
    A-->>U: pending ⌛
    
    Note over U: ちょっと待つ (Task.Delay)
    
    U->>A: 2. Status 取得 (opId)
    A-->>U: pending ⌛
    
    Note over U: ちょっと待つ
    
    U->>A: 3. Status 取得 (opId)
    A-->>U: done ✅
