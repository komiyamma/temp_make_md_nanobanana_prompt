...
```mermaid
flowchart TD
  UX["ユーザー体験😊"] --> LCP["LCP: 表示の速さ🏃‍♀️"]
  UX --> INP["INP: 操作の反応🖱️"]
  UX --> CLS["CLS: ガタガタしない📐"]

  LCP --> C1["主に画像/フォント/サーバー応答🖼️🔤⚡"]
  INP --> C2["主にJSの重さ/長い処理🐘🧠"]
  CLS --> C3["主にサイズ未指定/後出しUI📦↕️"]
```
...
```mermaid
flowchart LR
  Lab["ラボ計測🧪<br>Lighthouseなど"] --> Hint["改善のヒント🔎"]
  Field["実ユーザー計測🌍<br>Search Console/CrUX/PSI"] --> Judge["合否の判断✅"]
  Hint --> Fix["直す✏️"]
  Fix --> Deploy["デプロイ🚀"]
  Deploy --> Field
```
