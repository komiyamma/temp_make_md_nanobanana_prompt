...
![全体構成図](./picture/es_cs_study_036_total_view.png)

```mermaid
graph TD
    User([ユーザー 📱]) -- HTTP --> API[Minimal API 🌐]
    ...
    subgraph Storage [SQLite 🗄️]
        ES[(EventStore  Tape)]
        PS[(Projection 🍱)]
    end
    
    API -- "1. Load / Append" --> ES
    API -- "2. Apply" --> PS
    API -- "Get View" --> PS
    
    Cart -- "Apply / Decide" --> Events
```
...
![プロジェクトのフォルダ構成](./picture/es_cs_study_036_folder_structure.png)
...
sequenceDiagram
    ...
    participant D as Domain (Cart) 🧠
    participant P as Projector 🍱

    U->>A: 1. Request (AddItem)
    A->>E: 2. ReadStreamAsync
    E-->>A: Events, lastVersion
    
    A->>D: 3. Rehydrate (Apply events)
    A->>D: 4. DecideAddItem (Command)
    D-->>A: NewEvents
    
    A->>E: 5. AppendAsync (expected=lastVersion)
    Note over E: 競合チェック & 保存
    
    A->>P: 6. ApplyAsync (Project)
    P-->>A: 完了
