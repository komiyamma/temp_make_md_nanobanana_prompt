...
![グローバルCSSの適用図](./picture/next_study_051_global_css.png)

```mermaid
flowchart TB
  subgraph APP["app/"]
    L["layout.tsx（RootLayout）🧱"]
    G["globals.css（グローバルCSS）🌍"]
    P["page.tsx / 各ルート配下のページたち📄"]
  end
...
```mermaid
flowchart LR
  A["globals.css 🌍（全体ルール）"] --> B["body / html / 共通レイアウト"]
  C["xxx.module.css 🧩（局所ルール）"] --> D["特定コンポーネントや一部画面だけ"]
```
