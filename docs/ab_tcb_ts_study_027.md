...
```mermaid
sequenceDiagram
    participant User as ユーザー (Aさん) 🧑‍💻
    participant S1 as サーバー (1回目) 🗄️
    participant S2 as サーバー (2回目) 🗄️
    participant Pay as 外部決済サービス 💳
    
    User->>S1: 支払い確定依頼
    User->>S2: 支払い確定依頼 (連打orリトライ)
    
    S1->>Pay: 1. 決済処理の呼び出し
    Note right of Pay: 決済成功 ✅
    S2->>Pay: 2. 決済処理の呼び出し <br/>(キー無しだと通ってしまう!)
    Note right of Pay: 二重課金!! 💸💸💥
    
    S1->>S1: 3. 注文をPaidに更新 (DB保存 OK)
    S2->>S2: 4. 注文をPaidに更新 (DB保存 Lockで失敗) ❌
    
    Note over User: DBは守られたけど、お金は2回引かれた...😭
```
