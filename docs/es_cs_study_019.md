...
sequenceDiagram
    ...
    H->>S: 1. Load: 既存の履歴を読む 📚
    S-->>H: イベント列 (v1, v2...)
    H->>A: 2. Rehydrate: 現在状態を復元 🧠
    H->>A: 3. Decide: 差分イベントを生成 🔧
    A-->>H: New Event(s) 📜
    H->>S: 4. Append: 履歴の末尾に追加 🧾
    S-->>H: 完了
    H-->>U: 成功レスポンス ✅
```
...
```mermaid
flowchart TD
    Cmd[ICommand 受信] --> Switch{Type Switch}
    
    Switch -- CreateCart --> D_Create[DecideCreate]
    Switch -- AddItem --> D_Add[DecideAddItem]
    Switch -- ChangeQuantity --> D_Change[DecideChangeQuantity]
    Switch -- RemoveItem --> D_Remove[DecideRemoveItem]
    
    subgraph Validation ["共通チェック 🛡️"]
        D_Create & D_Add & D_Change & D_Remove --> CheckCreated[作成済み/未作成チェック]
        D_Add & D_Change & D_Remove --> CheckSku[SKU 有効性チェック]
    end
    
    CheckCreated & CheckSku --> Logic[固有の業務ルール判定]
    Logic --> Result[新イベント 📜 / エラー 🛑]
```
