...
```mermaid
flowchart TD
    Fetch["fetch('http://api...')"] --> Result{結果は？}
    Result -- "Network Error\n(DNS/Timeout)" --> Reject["Reject 💥\n(catchへ)"]
    Result -- "HTTP 404/500" --> Resolve["Resolve ✅\n(ok: false)"]
    Result -- "HTTP 200" --> ResolveOK["Resolve ✅\n(ok: true)"]
    
    Resolve --> Check{res.ok Check}
    Check -- false --> ManualErr[自分で Error を投げる]
```
