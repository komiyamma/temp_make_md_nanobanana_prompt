...
![性能の3大指標](./picture/es_cs_study_027_perf_metrics.png)

```mermaid
graph LR
    subgraph Metrics [性能の 3 大指標]
        M1["1. レイテンシ ⏱️<br/>(Latency)"]
        M2["2. スループット 🚚<br/>(Throughput)"]
        M3["3. メモリ/GC 🧹<br/>(Alloc/GC)"]
    end
    
    M1 --- M1_d["1回の処理時間<br/>(応答性)"]
    M2 --- M2_d["時間あたりの処理件数<br/>(処理能力)"]
    M3 --- M3_d["メモリ確保量と掃除回数<br/>(安定性)"]
    
    style Metrics fill:#f9f9f9,stroke:#333
```
...
![パフォーマンス向上の黄金ループ](./picture/es_cs_study_027_golden_loop.png)

```mermaid
flowchart TD
    S1[1. シナリオ決定 🎬] --> S2[2. 指標決定 📌]
    S2 --> S3[3. 現状計測 📸]
    S3 --> S4[4. 仮説立案 🕵️‍♀️]
    S4 --> S5[5. 1点改善 ✂️]
    S5 --> S6[6. 再計測 🔁]
