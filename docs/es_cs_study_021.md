...
```mermaid
flowchart TD
    Error[失敗/例外が発生] --> IsDomain{仕様として <br/>想定内か? 🧭}
    
    IsDomain -- Yes: ドメインエラー --> Result["Result.Fail(DomainError) を返す 🚦"]
    IsDomain -- No: インフラエラー --> Throw["例外 (Exception) を投げる 🧯"]
...
![Result型の使い分け](./picture/es_cs_study_021_result_traffic.png)

```mermaid
graph LR
    subgraph Traffic ["Result型の 3 色信号 🚦"]
        direction LR
        Blue((青)) --- Ok["正常終了 <br/>(Success)"]
        Yellow((黄)) --- Dom["ドメインエラー <br/>(DomainError)"]
        Red((赤)) --- Infra["インフラエラー <br/>(Exception)"]
    end
...
```mermaid
graph LR
    subgraph Result_Layer [Result / Domain]
        R1[validation_error]
        R2[not_found]
        R3[conflict]
        R4[rule_broken]
    ...
    subgraph HTTP_Layer [Web API / Problem Details]
        H1["400 Bad Request"]
        H2["404 Not Found"]
        H3["409 Conflict"]
        H4["422 Unprocessable Entity"]
    end
