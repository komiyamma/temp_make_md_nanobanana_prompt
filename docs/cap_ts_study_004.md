...
![ネットワーク故障](./picture/cap_ts_study_004_network_fault.png)
...
sequenceDiagram
    ...
    C->>N: 1. リクエスト送信
    N->>S: 2. サーバーに到達
    Note over S: 3. 処理中...(遅い)
    Note over C: 4. タイムアウト判定⏰
    C-->>C: 諦めて次の処理へ
    Note over S: 5. 処理完了！✅
    S->>N: 6. レスポンス送信
    N--xC: (クライアントはもう聞いてない)
...
sequenceDiagram
    ...
    Note over A: 処理開始 (delay=2500ms)
    W-->>W: 800ms経過: タイムアウト⏰
    W->>A: attempt=2 (再送)
    Note over A: 1回目完了 (ChargedTimes=1)
    Note over A: 2回目完了 (ChargedTimes=2) 💥
