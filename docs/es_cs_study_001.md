...
![ES/CS 学習ロードマップ](./picture/es_cs_study_001_roadmap.png)
...
```mermaid
graph LR
    subgraph StateBased ["状態保存 (State-based)"]
        S1[現在: カートに3個]
    end
    subgraph EventSourcing ["イベントソーシング (ES)"]
        E1[追加: A] --> E2[追加: B] --> E3[削除: A] --> E4[追加: C]
    end
    EventSourcing -- "履歴を再生 (Replay)" --> StateBased
```
...
```mermaid
flowchart TD
    A[1. 言葉と芯をそろえる] --> B[2. 最小実装で動かす]
    B --> C[3. 復元で「なるほど！」]
    C --> D[4. Command処理の型を固める]
    D --> E[5. テストの型で安心]
    E --> F[6. 現実の壁へ挑戦]
